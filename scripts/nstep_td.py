...
class NstepTD:
    def __init__(self, env: BaseEnv, alpha=0.5, gamma=1.0, epsilon=0.1, n_episodes=100, n=5, excepted_sarsa=False):
        self.env = env
        self.alpha = alpha
        self.gamma = gamma
        ...
        for episode in range(self.n_episodes):
            ...
            while True:
                ...
                if tau >= 0:
                    ...
                    RHO = np.prod(rhos[tau + 1:min(tau + self.n, T)])
    ...
    def tree_backup(self):
        actions_ids = list(range(self.env.n_actions))
        for episode in range(self.n_episodes):
            state = self.env.start_state
            ...
            while True:
                ...
                if tau >= 0:
                    ...
                    for k in range(min(t, T - 1), tau, -1):
                        pq = [self.get_action_probability(states[k], a) * self.Q[states[k]][a] for a in actions_ids if a != actions[k]]
                        G = rewards[k] + self.gamma * sum(pq)
                        G += self.gamma * self.get_action_probability(states[k], actions[k])
